...
class WerewolfServer:
    ...
    def player_snapshot(self, player_id):
        ...
        return {
            ...
            "night_target_name": self.players[self.pending_wolf_target]["name"] if self.pending_wolf_target is not None and current_role == "Sorciere" else None,
            "can_act": can_act,
    ...
    def resolve_wolves_if_ready(self):
        wolves = [p for p in self.players if p.get("connected") and p["alive"] and p["role"] == "Loup-garou"]
    ...
    def resolve_night_if_ready(self):
        ...
        witch_ready = all(p["role"] != "Sorciere" or not p["alive"] or self.pending_night.get("witch_done", False) for p in self.players if p.get("connected"))
    ...
    def handle_night_action(self, player_id, msg):
        ...
        if action == "witch_save":
            if player["role"] != "Sorciere" or self.pending_night.get("witch_done", False):
