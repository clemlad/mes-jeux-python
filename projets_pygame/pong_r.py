...
duree_special_rachid = 5400  # Valeur par défaut de 1h30
parametres_partie = {}
...
def menu_special_rachid():
    """Affiche le menu de sélection de la durée pour le mode Spécial Rachid (1h30, 3h ou 4h30)."""
    global temps_de_compte_a_rebours, mode_jeu, duree_special_rachid
    while True:
        fenetre.fill(noir)
        titre = police.render("Durée du match Spécial Rachid", True, blanc)
        fenetre.blit(titre, (largeur // 2 - titre.get_width() // 2, hauteur // 2 - 150))
        bouton_1h30 = bouton_texte("1h30", largeur // 2 - 200, hauteur // 2 - 30)
        bouton_3h = bouton_texte("3h", largeur // 2 + 50, hauteur // 2 - 30)
        bouton_4h30 = bouton_texte("4h30", largeur // 2 - 75, hauteur // 2 + 60)
        bouton_retour = bouton_texte("Retour", largeur - 200, hauteur - 80)
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                sys.exit()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if bouton_1h30.collidepoint(event.pos):
                    duree_special_rachid = 5400
                    mode_jeu = "special_rachid"
                    temps_de_compte_a_rebours = duree_special_rachid
                    choisir_nombre_joueurs()
                    return
                elif bouton_3h.collidepoint(event.pos):
                    duree_special_rachid = 10800
                    mode_jeu = "special_rachid"
                    temps_de_compte_a_rebours = duree_special_rachid
                    choisir_nombre_joueurs()
                    return
                elif bouton_4h30.collidepoint(event.pos):
                    duree_special_rachid = 16200
                    mode_jeu = "special_rachid"
                    temps_de_compte_a_rebours = duree_special_rachid
                    choisir_nombre_joueurs()
                    return
...
def menu_mode():
    """Affiche le menu principal de sélection du mode : Classique, Endurance ou Spécial Rachid."""
    global mode_jeu
    while True:
        fenetre.fill(noir)
        titre = police.render("Choisissez le mode de jeu", True, blanc)
        fenetre.blit(titre, (largeur // 2 - titre.get_width() // 2, hauteur // 2 - 200))
        bouton_classique = bouton_texte("Classique", largeur // 2 - 200, hauteur // 2 - 80)
        bouton_endurance = bouton_texte("Endurance", largeur // 2 + 50, hauteur // 2 - 80)
        bouton_special = bouton_texte("Spécial Rachid", largeur // 2 - 75, hauteur // 2 + 20)
        bouton_quitter = bouton_texte("Quitter", largeur // 2 - 75, hauteur // 2 + 120)
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if bouton_classique.collidepoint(event.pos):
                    ...
                    choisir_nombre_joueurs()
                    return
                elif bouton_special.collidepoint(event.pos):
                    menu_special_rachid()
                    return
...
def initialiser_partie():
    """Réinitialise les scores, repositionne les raquettes et adapte le temps selon le mode de jeu."""
    global score1, score2, temps_restant, parametres_partie, raquette1, raquette2
    score1 = 0
    score2 = 0
    if mode_jeu == "endurance":
        temps_restant = 300  # 5 minutes
    elif mode_jeu == "special_rachid":
        temps_restant = duree_special_rachid
    else:
        temps_restant = temps_de_compte_a_rebours
...
while True:
    ...
    if not jeu_en_cours:
        continue
    if mode_jeu in ["endurance", "special_rachid"] and not pause:
        temps_restant -= 1 / 60
        if temps_restant <= 0:
            verifier_victoire()
            continue
    ...
    if mode_jeu in ["endurance", "special_rachid"]:
        heures = int(temps_restant) // 3600
        minutes = (int(temps_restant) % 3600) // 60
        secondes = int(temps_restant) % 60
        temps_affiche = police.render(f"{heures}:{minutes:02d}:{secondes:02d}", True, blanc)
